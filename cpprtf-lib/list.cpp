    ...
    for(vector<BaseElement*>::iterator it = m_items.begin();it!=m_items.end();it++ ) {
        ...
        if (item->instanceOf<CppRtf_List_Numbering*>() ) {
            if (this->instanceOf<CppRtf_List_Numbering*>()) {
                CppRtf_List_Numbering* numbering = (CppRtf_List_Numbering*)item->get();
        ...
        if (!(item->instanceOf<CppRtf_List*>()||item->instanceOf<CppRtf_List_Enumeration*>()||item->instanceOf<CppRtf_List_Numbering*>())) {
